...
// Neither → a self-signed-up account with no access. This only picks the UI;
// the policies in 034_subcontractor_portal.sql are what enforce it.
export function usePortalUser(userId) {
  const [state, setState] = useState({ loading: !!userId, portalUser: null, isStaff: true });

  useEffect(() => {
    if (!userId) { setState({ loading: false, portalUser: null, isStaff: true }); return; }
    let cancelled = false;
    setState((s) => ({ ...s, loading: true }));
    Promise.all([
      supabase.from('subcontractor_portal_users').select('*').eq('user_id', userId).maybeSingle(),
      supabase.rpc('is_staff'),
    ]).then(([{ data: portalUser }, { data: isStaff, error: staffErr }]) => {
      if (cancelled) return;
      ...
      setState({ loading: false, portalUser: portalUser || null, isStaff: staffErr ? true : isStaff === true });
    });
