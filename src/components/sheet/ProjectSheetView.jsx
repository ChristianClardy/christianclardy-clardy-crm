            ...
            <tbody>
              {displayRows.map((row) => {
                ...
                return <tr key={row.id} data-row-id={row.id} className={cn("border-b border-slate-100 group transition-colors group/trow cursor-pointer", selectedRowId === row.id && !isRowSelected && "ring-1 ring-inset ring-amber-300")} style={{ height: sheetStyle.rowHeight, backgroundColor: rowBg }} draggable={!sortConfig.col} onClick={() => { if (!selection) setSelectedRowId(row.id === selectedRowId ? null : row.id); }} onDragStart={(e) => onDragStart(e, row.id)} onDragEnter={(e) => onDragEnter(e, row.id)} onDragEnd={onDragEnd} onMouseEnter={() => setHoveredRow(row.id)} onMouseLeave={() => setHoveredRow(null)}>
                  <td className={cn("px-1 py-1 sticky left-0 z-10 w-8 min-w-[32px] select-none", isRowSelected ? "bg-blue-100" : "bg-inherit")} style={{ backgroundColor: isRowSelected ? "#bfdbfe" : rowBg }} onClick={(e) => { e.stopPropagation(); const shiftKey = e.shiftKey; const multiKey = e.ctrlKey || e.metaKey; const rowId = row.id; startTransition(() => { setSelectedRowIds(prev => { const next = new Set(prev); if (shiftKey) { const lastSelected = [...next].pop(); const from = taskDisplayRowIds.indexOf(lastSelected); const to = taskDisplayRowIds.indexOf(rowId); if (from !== -1 && to !== -1) taskDisplayRowIds.slice(Math.min(from, to), Math.max(from, to) + 1).forEach(id => next.add(id)); else next.has(rowId) ? next.delete(rowId) : next.add(rowId); } else if (multiKey) { next.has(rowId) ? next.delete(rowId) : next.add(rowId); } else { if (next.size === 1 && next.has(rowId)) return new Set(); return new Set([rowId]); } return next; }); setSelectedColKeys(new Set()); }); }} title="Click to select row for formatting"><div className="flex items-center justify-end gap-0.5"><span className={cn("text-[10px] w-5 text-right select-none", isRowSelected ? "text-blue-600 font-bold" : "text-slate-300 group-hover:hidden")}>{taskRowNumMap[row.id]}</span><GripVertical className={cn("w-3.5 h-3.5 text-slate-300", isRowSelected ? "hidden" : "hidden group-hover:block")} /></div></td>
